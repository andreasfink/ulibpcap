...
static uint32_t calculate_crc32c(uint32_t crc32c,const unsigned char *buffer, unsigned int length);
static uint32_t finalize_crc32c(uint32_t crc32c);
              ...
              fragment:(int)fragment
{
    ...
    header[0] = 0x45; /*  , header version = 4, header length = 5*4 20 bytes */
    header[1] = dscp;
       ...
       fragmentLength:(uint16_t)fragmentLength
{
    ...
    NSMutableData *tpkt = [[NSMutableData alloc]initWithBytes:h length:sizeof(h)];
    [tpkt appendData:tcpPayload];
    
    int tcpChecksum = [UMPCAPMirrorPort layer4_checksum:tcpPayload
                                              headerPtr:&h[0]
                                              headerLen:sizeof(h)
                                               sourceIp:srcIp
                                          destinationIp:dstIp
      ...
      destinationPort:(uint16_t)destinationPort
{
    ...
    h[6] = 0;
    h[7] = 0;
    int udpChecksum = [UMPCAPMirrorPort layer4_checksum:udpPayload
                                              headerPtr:&h[0]
                                              headerLen:sizeof(h)
                                               sourceIp:srcIp
                                          destinationIp:dstIp
           ...
           verificationTag:(uint32_t)ver
{
    ...
    h[10] = 0;
    h[11] = 0;
    NSMutableData *packet = [[NSMutableData alloc]initWithBytes:h length:sizeof(h)];
    for(NSMutableData *chunk in chunks)
    {
    ...
    uint32_t sctpChecksum = [UMPCAPMirrorPort calculateCRC32c:packet];
    h[8] = (sctpChecksum >> 24) & 0xFF;
    h[9] = (sctpChecksum >> 16) & 0xFF;
    h[10] = (sctpChecksum >> 8) & 0xFF;
    h[11] = (sctpChecksum >> 0) & 0xFF;
    
    packet = [[NSMutableData alloc]initWithBytes:h length:sizeof(h)];
    for(NSMutableData *chunk in chunks)
    {
        while((chunk.length % 4) != 0)
        {
...
+ (uint32_t) calculateCRC32c:(NSData *)data
{
...
#pragma mark    -
#pragma mark    CRC32C calculations
...
#else
        term1 = crc_tableil8_o56[(*(const uint32_t *) p_buf) & 0x000000FF] ^ crc_tableil8_o48[((*(const uint32_t *) p_buf) >> 8) & 0x000000FF];
...
static uint32_t singletable_crc32c(uint32_t crc32c, const unsigned char *buffer, unsigned int length)
{
    ...
    for (i = 0; i < length; i++)
    {
        crc32c= (crc32c >> 8) ^ crc_c [(crc32c^(buffer[i]))&0xFF];
    }
    return (crc32c);
}
